...
import { Course, CourseStatus, User, UserStats, LearningPath, Badge } from '../types';
...
export const Dashboard: React.FC<DashboardProps> = ({ 
  ...
  const InsightCard = ({ label, value, subtext, icon: Icon, color, trend, trendDirection = 'up' }: any) => (
    <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm flex flex-col justify-between h-32 hover:shadow-md transition-shadow">
      <div className="flex justify-between items-start">
        <div className={`p-2 rounded-xl bg-opacity-10 ${color.bg} ${color.text}`}>
          <Icon size={20} />
        </div>
        {trend && (
          <span className={`text-xs font-semibold px-2 py-1 rounded-full flex items-center gap-1 ${
             trendDirection === 'up' ? 'text-[#008080] bg-[#008080]/10' : 'text-red-600 bg-red-50'
          }`}>
      ...
      <div>
        <div className="text-2xl font-bold text-slate-800 mt-2 truncate">{value}</div>
        <div className="text-xs text-slate-500 font-medium mt-1 truncate">{label}</div>
      </div>
    </div>
  );
            ...
            <div key={i} className="flex flex-col items-center gap-2 flex-1 group cursor-pointer">
              <div className="w-full relative h-32 bg-slate-50 rounded-lg overflow-hidden flex items-end">
                <div 
                  ...
                  style={{ height: `${(val / maxVal) * 100}%`, opacity: val === 0 ? 0 : 1 }}
                ></div>
                {/* Tooltip */}
              ...
              <span className="text-xs font-medium text-slate-400 group-hover:text-[#008080]">{days[i]}</span>
            </div>
          ))}
        </div>
      </div>
    ...
    return (
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden h-full flex flex-col">
        <div className="p-6 border-b border-slate-50 bg-slate-50/50">
          ...
          <p className="text-sm text-slate-500 mt-1 line-clamp-1">{path.description}</p>
        </div>
        ...
        <div className="p-6 flex-1 overflow-y-auto custom-scrollbar">
          <div className="relative space-y-0">
             ...
             {path.steps.map((step, idx) => {
                ...
                return (
                  <div key={step.id} className={`relative z-10 flex gap-4 pb-8 last:pb-0 group ${isLocked ? 'opacity-50' : ''}`}>
                    ...
                    <div className={`flex-1 rounded-xl p-4 border transition-all ${isCurrent ? 'bg-[#008080]/10 border-[#008080]/2 shadow-sm' : 'bg-white border-[#008080]/2'}`}>
                       <div className="flex justify-between items-start">
                          <div>
                            <div className="text-xs font-medium text-slate-400 mb-1">STEP {idx + 1} • {step.type.toUpperCase()}</div>
                            <h4 className={`font-bold ${isCurrent ? 'text-[#008080]' : 'text-slate-800'}`}>{step.title}</h4>
                            <p className="text-sm text-slate-500 mt-1">{step.description}</p>
                          </div>
                          {step.estimatedTime && <span className="text-xs text-slate-400 bg-white px-2 py-1 rounded border border-[#008080]/2 whitespace-nowrap">{step.estimatedTime}</span>}
                       </div>
    ...
    return (
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 flex flex-col h-full mt-lg-2">
         <div className="flex justify-between items-center mb-6">
            ...
            <span className="text-xs font-medium text-blue-600 hover:underline cursor-pointer">View All</span>
         </div>
         ...
         <div className="flex-1 flex gap-4 overflow-x-auto pb-4 custom-scrollbar">
            {badges.map(badge => {
              ...
              return (
                <div key={badge.id} className={`flex-shrink-0 w-32 p-4 rounded-xl border flex flex-col items-center text-center gap-3 transition-colors ${badge.isLocked ? 'bg-slate-50 border-slate-100 opacity-60' : 'bg-yellow-50/30 border-yellow-100'}`}>
                   <div className={`w-12 h-12 rounded-full flex items-center justify-center ${badge.isLocked ? 'bg-slate-200 text-slate-400' : 'bg-yellow-100 text-yellow-600 shadow-sm'}`}>
                      {badge.isLocked ? <Lock size={20} /> : <Icon size={24} />}
                   </div>
                   <div>
                     <div className="text-xs font-bold text-slate-800 leading-tight mb-1">{badge.name}</div>
                     <div className="text-[10px] text-slate-500 leading-tight line-clamp-2">{badge.description}</div>
                   </div>
                </div>
              );
            })}
        ...
        <div className="space-y-6 flex flex-col h-full">
           ...
           <div className="bg-gradient-to-br from-[#008080]/50 to-[#008080] rounded-2xl p-6 text-white shadow-lg flex-1 flex flex-col justify-center">
              <div className="flex items-start justify-between mb-4">
                 ...
                 <span className="text-indigo-100 text-xs font-medium uppercase tracking-wider">Recommended</span>
              </div>
              <h4 className="font-bold text-lg mb-2">Van Sales Representative in FMCG</h4>
              <p className="text-indigo-100 text-sm mb-4 line-clamp-2">Master the soft skills required for leadership roles.</p>
              <button className="bg-white text-[#008080] py-2 rounded-lg text-sm font-bold hover:bg-indigo-50 transition-colors w-full">
                View Details
              </button>
           </div>
        </div>
          ...
          {inProgress.length > 0 ? (
            <div className="h-full flex flex-col">
               <div className="flex justify-between items-center mb-4">
                 <h3 className="text-xl font-bold text-slate-800">Jump Back In</h3>
                 <button className="text-blue-600 text-sm font-medium hover:underline">View Library</button>
               </div>
