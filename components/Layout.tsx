...
import { LayoutDashboard, BookOpen, UserCircle, Wifi, WifiOff, Award, MessageCircle } from 'lucide-react';
      ...
      <aside className="hidden md:flex w-24 bg-white border-r border-slate-200 flex-col items-center py-6 shrink-0 fixed h-full z-10">
        ...
        <div className="mt-auto flex flex-col items-center gap-4 w-full">
           <button 
             ...
             className={`w-10 h-10 rounded-full flex items-center justify-center transition-colors ${isOffline ? 'bg-orange-100 text-orange-600' : 'bg-slate-100 text-green-600'}`}
             title={isOffline ? "Currently Offline" : "Online"}
           >
             {isOffline ? <WifiOff size={20} /> : <Wifi size={20} />}
           </button>
           <img src={user.avatar} alt="User" className="w-10 h-10 rounded-full border-2 border-slate-100" />
        </div>
      </aside>
      ...
      <main className="flex-1 md:ml-24 pb-20 md:pb-0">
         <header className="bg-white/80 backdrop-blur-md sticky top-0 z-20 px-6 py-4 border-b border-slate-100 flex justify-between items-center">
            ...
            <div className="flex md:hidden items-center gap-3">
               <button 
                 onClick={toggleOffline}
                 className={`p-2 rounded-full ${isOffline ? 'bg-orange-100 text-orange-600' : 'bg-green-50 text-green-600'}`}
               >
